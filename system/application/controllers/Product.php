...
class Product extends CI_Controller
{
    ...
    public function process_multi_delivery()
    {
        ...
        if ($selected_ids) {
            ...
            foreach ($id_array as $id) {
                $orderid       = $this->db_model->select('orderid', 'product_sale', array('id' => $id));
            ...
            $this->session->set_flashdata('common_flash', '<div class="alert alert-success">No orders selected.</div>');
        }
    ...
    public function add_product()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $distributing_price     = $this->common_model->filter($this->input->post('distributing_price'), 'float');
            ...
            $sponser_level_inc       = $this->input->post('sponser_level_inc');
            $show_on_repurchase        = $this->input->post('show_on_repurchase');
            ...
            $data = array(
                ...
                'distributing_price'    => $distributing_price,
                'franchisee_price'      => $this->input->post('franchisee_price'),
                'fran_inc'              => $this->input->post('franchisee_inc'),
                ...
                'roi_limit'             => $roi_limit,
                'status'                => 'Selling',
                'sponser_level_inc'     => $sponser_level_inc,
                'show_on_repurchase'    => $show_on_repurchase,
                'upline_fran_inc'  => $this->input->post('upline_franchisee_inc'),
            );
    ...
    public function remove($id)
    {
        ...
        if ($count > 0) {
            ...
            if($id == 3)
            {
                $this->session->set_flashdata('common_flash', '<div class="alert alert-danger">Deletion failed: This product is a free joining product and cannot be removed.</div>');
                redirect('product/manage_products');
            }
    ...
    public function search() 
    {
        ...
        $this->db->select('id, prod_name, prod_price, gst, image, qty, sold_qty, show_on_regform')->order_by('prod_name', 'ASC');
        if ($category !== "All") {
    ...
    public function pending_orders_report()
    {
        ...
        $this->db->where('status', 'Processing')->order_by('date', 'ASC');
    ...
    public function pending_orders()
    {
        ...
        $type2      = $this->input->post('type2') ?? NULL;
    ...
    {
        $this->db->select('*')->from('product_sale')->where(array('orderid'=> $id));
        $order_data     = $this->db->get()->result();
        $data['layout'] = 'product/view_order.php';
        $data['title']  = 'Order details of orderid '.$id;
