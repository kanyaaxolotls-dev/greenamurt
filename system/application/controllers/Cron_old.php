...
public function index()
    {
        // १. आधी सर्व इन्कम कॅल्क्युलेशन रन करा (Earning Table मध्ये एन्ट्री होतील)
        $this->newcron2();
        ...
        // २. DAILY MATCHING CUTOUT (दररोज रात्री १२ ला)
        // हा विभाग फक्त 'Matching Income' प्रोसेस करेल
        $this->process_automated_payout('Matching Income');

        // ३. MONTHLY PAYOUT (महिन्याच्या शेवटच्या दिवशी)
        // हा विभाग Sponsor, Level, Repurchase इ. सर्व प्रोसेस करेल
        if (date('t') == date('j')) 
        { 
            $this->process_automated_payout('Others'); 
            
            // Royalty Income प्रोसेस (महिन्याच्या १ तारखेला किंवा शेवटच्या दिवशी)
            $this->load->model('earning');
            $this->earning->royalty_income_process_month(date('Y-m'));
        }
        ...
        if ($this->input->is_cli_request()) {
            ...
            $this->session->set_flashdata('common_flash', '<div class="alert alert-success">Payout / Cron Executed Successfully!</div>');
...
//     }
 ...
 public function newcron2()
    {
        $this->update_legs(); // लेग्स अपडेट करणे
        $this->binary_payout(); // मॅचिंग इन्कम कॅल्क्युलेशन
        $this->reward(); // रिवॉर्ड कॅल्क्युलेशन
    }
    
     private function process_automated_payout($mode)
    {
        ...
        if ($mode == 'Matching Income') {
            $this->db->where('type', 'Matching Income');
        } else {
            // मॅचिंग सोडून बाकी सर्व (Sponsor, Level, Repurchase, Rewards)
            $this->db->where('type !=', 'Matching Income');
        }
        ...
        foreach ($groups as $grp) 
        {
            ...
            // --- FREE USER साठी ५००० वजावटीचे लॉजिक ---
            if (strtolower(trim($activation_type)) == 'free') 
            {
                if ($total_balance >= 5000) {
                    $this->db->insert('earning_deduct', [
                        'userid' => $userid,
                        'amount' => 5000,
                        'type'   => 'Auto Activation from Payout',
                        'date'   => date('Y-m-d')
                    ]);

                    $this->db->where('id', $userid)->update('member', [
                        'topup' => 5000,
                    ...
                    continue; // ५००० पूर्ण होईपर्यंत पैसे होल्डवर राहतील
                }
            }

            // --- फायनल पेआउट ट्रान्झॅक्शन ---
            if ($total_balance > 0) {
                $social_fund  = $total_balance * ($social_welfare_percent / 100);
                $final_payout = $total_balance - $social_fund;

                // १. स्टेटस अपडेट करा (Pending -> Paid)
                $this->db->where('userid', $userid)->where('status', 'Pending');
                if ($mode == 'Matching Income') {
                    $this->db->where('type', 'Matching Income');
                } else {
                    $this->db->where('type !=', 'Matching Income');
                }
                $this->db->update('earning', ['status' => 'Paid', 'payout_date' => date('Y-m-d')]);

                // २. Withdraw Request मध्ये एन्ट्री करा
                ...
                // ३. Social Welfare Fund मध्ये एन्ट्री करा
                if ($social_fund > 0) {
                    $this->db->insert('social_welfare_fund', [
                        'userid' => $userid,
                        'amount' => $social_fund,
                        'type'   => $mode . ' - Social Welfare Deduction',
                        'date'   => date('Y-m-d')
                    ]);
                }
            }
