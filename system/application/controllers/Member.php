...
{ 
    ...
    { 
        ...
        // 2. QUIZ & E-PIN ACCESS RESTRICTION LOGIC
   ...
   public function answer_sheet() {
    ...
    public function earning_dashboard($value = null)
    {
        ...
        #
            ...
            /* Repurchase and Royalty data commented out
            ...
            #print_r($this->db->last_query());die();
            */
            $data['total_repurchase_inc'] = 0;
            $data['total_royalty_inc'] = 0;
            $data['monthly_repurchase_inc'] = 0;
            $data['monthly_royalty_inc'] = 0;
    ...
    /* ROI Earning endpoints commented out
...
################################################ Epin Activation ########################################
        ...
        if ($epin_value > 0 && $epin_value == $package_cost->prod_price) {
            ...
            // Ensure Quiz status is marked Pass
            $chk_quiz = $this->db->get_where('quiz_results', array('userid' => $this->session->user_id, 'status' => 'Pass'))->row();
            if(!$chk_quiz) {
