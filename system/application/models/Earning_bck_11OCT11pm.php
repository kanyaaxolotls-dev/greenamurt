...
class Earning extends CI_Model
{
   ...
   // print_r('Paid Pair: '.$paid_pair);print_r('<br>Min Pair : '.$pair_match); print_r('<br>Max Pair : '.$pair_max);print_r('<br>Min Paid Matching '.$min);die();
            ...
            //&& $capping > 0 added by skt 
            if ($min > $capping && $capping > 0) {
                $min = $capping;
            }
            # First Binary.
...
// print_r('Paid Pair: '.$paid_pair);print_r('<br>Min Pair : '.$pair_match); print_r('<br>Max Pair : '.$pair_max);print_r('<br>Min Paid Matching '.$min);print_r('<br>Capping :'.$capping);die();
    ...
    public function reward_process()
    {
        ...
        foreach ($reward as $res) {
            ...
            $this->db->select('id')->from('member')->where(array(
                ...
                'total_d >=' => $res->D,
                'total_e >=' => $res->E,
                'total_sponsor >=' => $res->no_of_sponsor,
                'join_time >= ' . $duration,
