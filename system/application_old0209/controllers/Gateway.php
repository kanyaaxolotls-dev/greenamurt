...
class Gateway extends CI_Controller
{
    ...
    { 
        $this->load->view('paygateway/index');
    }
    public function pay_response()
    {
        $this->load->view('paygateway/response');
    }
    ...
    public function status($pg)
    {
        ...
        if ($this->session->_type_ == "userid" && $status == "Paid") {
            ...
            redirect(site_url('site/failed_registration'));
             ...
             //var_dump($this->session->_type_);var_dump($status);die();
    ...
    public function instamojo_start()
    {
        ...
        if ($site == "") {
            echo '<p>&nbsp;</p><p>&nbsp;</p><div align="center">There are some erros with payment gateway. Please check whether you have filled phone1 no and email id or not. <a class="btn btn-danger" href="' . site_url('gateway/status/instamojo') . '">Cancel</a></div>';
