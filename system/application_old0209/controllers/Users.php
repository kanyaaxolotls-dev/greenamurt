...
public function upgrade(){
    ...
    public function topup_member()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $user_data = $this->db_model->select_multi('signup_package, mypv', 'member', array('id' => $userid));
            $prod_pv = 0;
            if (!empty($user_data->signup_package)) {
                $prod_pv = $this->db_model->select('pv', 'product', array('id' => $user_data->signup_package));
            }
            $data   = array(
                'topup' => $epin_value,
