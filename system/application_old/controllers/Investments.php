...
class Investments extends CI_Controller
{
    ...
    public function __construct()
    {
		...
		//$this->_PAYOUT_CYCLE = array(1,7,10,15,20,30);
    ...
    {
        if ($this->login->check_session() == FALSE) {
            exit('<h3 align="center">Oops !! Somthing wrong with system</h3>');
        }
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			//$data['payout_cycle_dropdown'] = $this->_PAYOUT_CYCLE;
            ...
            $array = array(
                ...
                'sp_level_income' => $this->input->post('sp_level_income'),
				...
				'is_working_days'   => $this->input->post('is_working_days')
    ...
    {
        if ($this->login->check_session() == FALSE) {
            exit('<h3 align="center">Your session out ! kindly re-login.</h3>');
        }

        $this->form_validation->set_rules('package_name', 'Package Name', 'trim|required');
		// $this->form_validation->set_rules('roi_limit', 'ROI Limit', 'trim|required');
        if ($this->form_validation->run() == FALSE) {
    ...
    public function approve_invstment($id)
    {
        ...
        $this->investment->inv_earning($userid, $this->db_model->select('sponsor', 'member', array('id' => $userid)), $id);
    ...
    public function client_investment()
    {
        $config['base_url']   = site_url('investments/invest_client_report');
        $config['per_page']   = 50;
        $config['total_rows'] = $this->db_model->count_all('investments', array(
            'status !=' => 'Expired',
            'userid'    => $this->session->user_id,
        ...
        $data['title']      = 'Recent Investments';
        $data['breadcrumb'] = 'Recent Investments';
        $data['layout']     = 'investment/invest_client_report.php';
        $this->load->view('member/index', $data);
