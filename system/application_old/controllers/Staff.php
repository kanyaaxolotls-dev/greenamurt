...
class Staff extends CI_Controller
{
    ...
    public function edit_des($id)
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $perm = array(
                ...
                'createEpin' => $this->input->post('createEpin') ? $this->input->post('createEpin') : '0',
                'editEpin' => $this->input->post('editEpin') ? $this->input->post('editEpin') : '0',
                'viewEpin' => $this->input->post('viewEpin') ? $this->input->post('viewEpin') : '0',
                'transferEpin' => $this->input->post('transferEpin') ? $this->input->post('transferEpin') : '0',
    ...
    {
        $edit_id = $this->input->post('edit_id') ?? null;
        ...
        if ($this->form_validation->run()) {
            $data = [
                ...
                'payscale' => $this->common_model->filter($this->input->post('payscale'), 'float'),
                'tasks'    => implode(',', $this->input->post('menu_permissions') ?? []),
            ];
    
            if ($edit_id) {
                $this->db->where('id', $edit_id)->update('tbl_roles', $data);
                $this->session->set_flashdata('common_flash', '<div class="alert alert-success">Designation updated.</div>');
            } else {
                $this->db->insert('tbl_roles', $data);
                $this->session->set_flashdata('common_flash', '<div class="alert alert-success">Designation created.</div>');
            }
        ...
        $config['base_url']   = site_url('staff/designations');
        $config['per_page']   = 10;
        $config['total_rows'] = $this->db_model->count_all('tbl_roles');
        $page                 = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
        $this->pagination->initialize($config);
    
        $this->db->limit($config['per_page'], $page);
        $data['result'] = $this->db->get('tbl_roles')->result();
    
        // Handle edit data
        if ($edit_id) {
            $data['edit_data'] = $this->db_model->select_multi('*', 'tbl_roles', ['id' => $edit_id]);
        }
    ...
    function new_staff()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $this->db->select('*')->from('tbl_roles')->where('id !=', 1);
    ...
    public function update() {
        ...
        if ($query->num_rows() > 0) {
            $this->session->set_flashdata('common_flash', '<div class="alert alert-danger">Username already exists. Please choose a different one.</div>');
