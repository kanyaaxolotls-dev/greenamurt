 ...
 * Usage:
 * - Browser: http://localhost/greenamrutayurveda/generate_direct_sponsor_log_now.php?userid=MEMBER_ID
...
$log_file = __DIR__ . '/system/application/logs/direct_sponsor_debug.log';
$root_log = __DIR__ . '/direct_sponsor_debug.log';
...
} elseif (file_exists($root_log)) {
    $content = file_get_contents($root_log);
    if (strpos($content, "DIRECT SPONSOR DEBUG") !== false) {
        $blocks = explode("DIRECT SPONSOR DEBUG", $content);
        $last_block = end($blocks);
        echo "DIRECT SPONSOR DEBUG" . $last_block;
